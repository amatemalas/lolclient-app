...
- **App icon/name**: app icons live in `public/` as `icon.png`, `icon.ico`, `icon.icns` (+ `IconTemplate*.png` for the tray). NativePHP copies them into the electron project's `build/` on every `native:run` / `native:build`. Regenerate them with `php scripts/generate-icons.php` (needs the `gd` extension). App display name = `APP_NAME` (`.env`), app id = `NATIVEPHP_APP_ID`.
- **Vendor patch (icon on Windows)**: `nativephp/desktop 2.2.1` only sets the `BrowserWindow` icon on Linux, so Windows dev shows the plain Electron icon. We patched `electron-plugin/src/server/api/window.ts` (+ `dist/server/api/window.js`) to include `win32` in the `process.platform` check. A `composer update` will restore the original.
