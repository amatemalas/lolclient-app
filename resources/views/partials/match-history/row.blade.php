...
<div class="group flex items-center gap-4 px-4 py-3 transition-colors hover:bg-steel-2/50">
    <div class="flex w-16 items-center gap-2">
        <span class="{{ $match['win'] ? 'bg-arcane/15 text-arcane-bright' : 'bg-ember/15 text-ember' }} clip-corner-sm px-2 py-1 text-[9px] font-black uppercase tracking-[0.16em]">
            {{ $match['win'] ? 'Win' : 'Def' }}
        </span>
    </div>

    <div class="clip-corner-sm relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden bg-gradient-to-br {{ $gradient }} ring-1 ring-line">
    ...
    <div class="ml-auto flex items-center gap-3">
        <span class="clip-corner-sm {{ $match['win'] ? 'bg-gold/15 text-gold-bright' : 'bg-ember/15 text-ember' }} px-2 py-0.5 font-display text-[11px] font-bold">Lv {{ $match['champLevel'] }}</span>
