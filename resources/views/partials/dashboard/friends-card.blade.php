...
<div class=" panel reveal flex min-h-0 flex-col p-5" style="--reveal-delay:.1s">
    <div class="flex items-center justify-between">
        ...
        <span class="bg-arcane/15 px-2 py-0.5 font-mono text-[10px] font-semibold text-arcane-bright">{{ $connected ? $onlineFriends.'/'.count($friends) : '—' }}</span>
    </div>
    ...
    <div class="mt-3 max-h-[380px] min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
        @forelse ($friends as $f)
            <div class="group flex items-center gap-3 rounded-sm border border-transparent px-2 py-2 transition-colors hover:border-line/60 hover:bg-steel-2/60">
                ...
                <span class="shrink-0 border px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.14em] {{ $f['pill'] }} {{ $f['text'] }}">{{ $f['status'] }}</span>
            </div>
        @empty
