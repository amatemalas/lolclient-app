...
@section('content')
    <div class="flex min-w-0 flex-1 flex-col items-center justify-center px-8 py-6">
        <div class="panel clip-corner reveal w-full max-w-xl px-10 py-10 text-center" style="--reveal-delay:.05s">
            ...
            <div class="mt-7 flex flex-wrap items-center justify-center gap-2 text-[10px] font-semibold uppercase tracking-[0.15em] text-mist">
                <span class="clip-corner-sm border border-line bg-steel-2/70 px-3 py-1.5">1 · Launch the client</span>
                <span class="text-gold/50">→</span>
                <span class="clip-corner-sm border border-line bg-steel-2/70 px-3 py-1.5">2 · Sign in</span>
                <span class="text-gold/50">→</span>
                <span class="clip-corner-sm border border-line bg-steel-2/70 px-3 py-1.5">3 · Return here</span>
            </div>

            <div class="mt-8 flex flex-wrap items-center justify-center gap-3">
                <a href="{{ url('/') }}"
                   class="clip-corner-sm inline-flex items-center gap-3 bg-gradient-to-br from-gold-bright via-gold to-gold-deep px-6 py-3 text-[12px] font-bold uppercase tracking-[0.2em] text-obsidian transition-transform hover:scale-[1.03]">
                    Retry connection
                    <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2">
                        <path d="M5 12h14M13 6l6 6-6 6" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </a>
                <button type="button" data-open-config
                        class="clip-corner-sm inline-flex items-center gap-3 border border-line px-6 py-3 text-[12px] font-bold uppercase tracking-[0.2em] text-mist transition-colors hover:border-gold/60 hover:text-cream">
                    Client settings
                </button>
            </div>
        </div>
    ...
    {{-- Client path configuration popup --}}
    <div data-config-modal role="dialog" aria-modal="true" aria-label="Client settings"
         class="fixed inset-0 z-40 hidden flex flex-col items-center justify-center px-6">
        <div data-config-backdrop class="absolute inset-0 bg-void/80 backdrop-blur-sm"></div>
        <div class="relative w-full max-w-2xl">
            <div class="mb-3 flex items-center justify-between">
                <p class="label text-gold-deep">Client settings</p>
                <button type="button" data-config-close aria-label="Close"
    ...
    <script>
        (function () {
            ...
            const opener = document.querySelector('[data-open-config]');
            if (!modal || !opener) {
                return;
            }

            const closeBtn = modal.querySelector('[data-config-close]');
            const backdrop = modal.querySelector('[data-config-backdrop]');
