...
const stats = computed(() => {
    const mc = props.matches.length;
    let sumK = 0, sumD = 0, sumA = 0, sumCs = 0, sumSec = 0;
    ...
    const kda = sumD > 0
        ? ((sumK + sumA) / sumD).toFixed(2)
        : mc > 0 ? 'Perfect' : '—';

    const csPerMin = sumSec > 0
        ? (sumCs / (sumSec / 60)).toFixed(1)
        : '—';

    return [
        { l: 'Win Rate', v: hasRank.value ? `${props.ranked.winRate}%` : '—' },
        { l: 'KDA', v: kda },
        { l: 'CS / Min', v: csPerMin },
        { l: 'Games', v: hasRank.value ? props.ranked.games.toLocaleString() : '—' },
    ];
});

const lpPercent = computed(() => Math.min(100, props.ranked.leaguePoints));
</script>
    ...
    <div class="panel reveal p-5">
        ...
        <div class="mt-4 flex items-center gap-4">
            ...
            <div class="min-w-0 flex-1">
                <p class="font-mono text-[13px] font-semibold text-cream">
                    {{ hasRank ? ranked.leaguePoints : '—' }} <span class="text-arcane-bright">LP</span>
                </p>
                <div class="mt-1.5 h-1.5 w-full overflow-hidden rounded-full bg-obsidian ring-1 ring-line/60">
                    <div class="h-full bg-gradient-to-r from-gold-deep via-gold to-gold-bright" :style="{ width: `${lpPercent}%` }"></div>
                </div>
                <p class="mt-1.5 text-[10px] text-mist">
                    <span v-if="hasRank && ranked.isProvisional" class="text-arcane-bright">
                        {{ ranked.provisionalGamesRemaining }} placement games left
                    </span>
                    <template v-else-if="hasRank">
                        {{ ranked.lpToNext }} LP to <span class="text-cream">{{ ranked.tier }} {{ ranked.nextDivision }}</span>
                    </template>
                    <template v-else>Play ranked to unlock</template>
                </p>
            </div>
        </div>

        <div class="mt-4 grid grid-cols-2 gap-2 border-t border-line/60 pt-4">
            <div v-for="s in stats" :key="s.l" class="rounded-sm bg-obsidian/70 px-3 py-2 ring-1 ring-line/60">
