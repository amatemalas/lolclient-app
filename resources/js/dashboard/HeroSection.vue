...
import SummonerAvatar from '../components/SummonerAvatar.vue';

const props = defineProps({
    summoner: { type: Object, required: true },
    ranked: { type: Object, required: true },
    connected: { type: Boolean, default: false },
    assetBase: { type: String, default: '/api/lcu/assets' },
});

const hasRank = computed(() => props.ranked.tier !== 'UNRANKED');
        ...
        <div class="relative flex flex-wrap items-center gap-8 p-7">
            <div class="flex items-center gap-5">
                <div class="relative">
                    ...
                    />
                    <span class="absolute -bottom-2 -right-2 bg-obsidian px-2 py-1 font-mono text-[11px] font-semibold text-gold-bright ring-1 ring-gold/50">
                        {{ summoner.summonerLevel.toLocaleString() }}
                    </span>
                </div>
                <div>
                    ...
                    <p class="mt-1 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.2em] text-mist">
                        ...
                        Summoner Level {{ summoner.summonerLevel.toLocaleString() }} · Season {{ year }}
                    </p>
                    <div class="mt-3 flex items-center gap-2">
                        ...
                        <span
                            class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em]"
                            :class="hasRank ? 'bg-gold/15 text-gold-bright' : 'bg-ember/15 text-ember'"
                        >{{ hasRank ? 'Clash Ready' : 'Unranked' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="ml-auto flex items-center gap-5">
                <div class="hidden text-right sm:block">
                    ...
                    <p class="mt-1 font-display text-2xl font-bold uppercase tracking-[0.1em] text-gold-grad">
                        {{ hasRank ? `${ranked.tier} ${ranked.division}` : 'Unranked' }}
                    </p>
                    <p class="font-mono text-[12px] text-mist">
                        <template v-if="hasRank">
                            {{ ranked.leaguePoints }} <span class="text-arcane-bright">LP</span>
                        </template>
                        <template v-else>Play placements</template>
                    </p>
                </div>
                <a
                    href="/lobby"
                    class="group relative bg-gradient-to-b from-gold-bright via-gold to-gold-deep px-10 py-3.5 font-display text-sm font-black uppercase tracking-[0.3em] text-obsidian shadow-[0_10px_30px_rgba(200,170,110,0.35)] transition-transform hover:-translate-y-0.5"
                    :class="{ 'pointer-events-none opacity-60': !connected }"
                >
                    Play
